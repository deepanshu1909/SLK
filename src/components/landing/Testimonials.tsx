...
export function SocialProof() {
  if (partnerLogos.length === 0) {
    return (
      <section className="py-12 border-y border-[var(--hairline)] bg-[var(--card)]">
        <div className="zk-container text-center">
          ...
          <p className="mt-2 zk-caption">
            Discovery, Conversion, Retention, and CRM — shaped for three distinct
  ...
  return (
    <section className="py-16 border-y border-[var(--hairline)] bg-[var(--card)]">
      <div className="zk-container">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-6 items-center opacity-70">
          {partnerLogos.map((l) => (
            <div key={l} className="font-display text-lg text-center text-[var(--heading)]">
              {l}
            </div>
          ))}
        </div>
