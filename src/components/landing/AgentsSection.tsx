    ...
    return (
      <ResponsiveContainer width="100%" height={48}>
        <LineChart data={revenueData}>
          <Line type="monotone" dataKey="revenue" stroke="oklch(0.58 0.22 282)" strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width="100%" height={48}>
        <BarChart data={trafficData}>
          <Bar dataKey="clicks" fill="oklch(0.7 0.16 220)" radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    );
  }
  return (
    <ResponsiveContainer width="100%" height={48}>
      <LineChart data={trafficData}>
        <Line type="monotone" dataKey="views" stroke="oklch(0.72 0.18 330)" strokeWidth={2} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
        ...
        <FadeIn>
          <div className="max-w-2xl">
            ...
            <h2 className="font-display text-5xl md:text-6xl leading-[1] tracking-tight">
              Six specialists. <br />
              <span className="italic text-gradient-brand">One team.</span>
            </h2>
          ...
          {agents.map((a, i) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="group relative rounded-3xl bg-card border p-7 shadow-card hover:shadow-elevated transition-shadow overflow-hidden"
              >
                {/* Glow on hover */}
                ...
                <div className="relative">
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-brand-soft border border-brand/20 grid place-items-center">
